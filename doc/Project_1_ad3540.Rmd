...
The first method is re-sampling the data$^2$. Essentially, this means taking less data from the over-represented classes and more from the under-represented classes. Since our data set is fairly large, we would use under-sampling for songs in the 2000's and 2010's. Another method of over-sampling is SMOTE, which stands for synthetic minority over-sampling technique. SMOTE generates fake samples in the minority class. A non-sampling method would be to add weight to the minority classes in the algorithm. 

In order to try and fix this data, I will use the under-sampling technique and re-run the KNN algorithm.
...
removed.rows.2000 <- sample(1:nrow(final_lyrics[final_lyrics$Decade == "2000", ]), #Randomly selecting rows
                       .50*nrow(final_lyrics[final_lyrics$Decade == "2000", ]))

removed.rows.2010 <- sample(1:nrow(final_lyrics[final_lyrics$Decade == "2010", ]),
                       .50*nrow(final_lyrics[final_lyrics$Decade == "2010", ]))


balanced_final_lyrics <- final_lyrics[-c(removed.rows.2000,removed.rows.2010),]
...
set.seed(1234)
train.rows <- sample(1:nrow(balanced_final_lyrics),.80*nrow(balanced_final_lyrics))

#Normalizing the numeric predictors and dummy code the genre variable

norm.balanced_final_lyrics <- balanced_final_lyrics

norm.balanced_final_lyrics$sentiment <- normalize(balanced_final_lyrics$sentiment) #Normalizing variables
norm.balanced_final_lyrics$Length <- normalize(balanced_final_lyrics$Length)

Genre.data <-as.data.frame(dummy.code(norm.balanced_final_lyrics$genre)) #Dummy coding genre 

norm.balanced_final_lyrics<-norm.balanced_final_lyrics[,-c(1:6)]
norm.balanced_final_lyrics<-cbind(norm.balanced_final_lyrics,Genre.data) #Combining all the processed variables back together


#Separating training set

lyrics_train <- norm.balanced_final_lyrics[train.rows,]

#Separating test set

lyrics_test <-  norm.balanced_final_lyrics[-train.rows,]

#Extract decade column since that is the response variable

lyrics_decade <-(balanced_final_lyrics[train.rows,"Decade"])
lyrics_train <- lyrics_train[,-1]


lyrics_test_decade <- (balanced_final_lyrics[-train.rows,"Decade"])
lyrics_test <- lyrics_test[,-1]
